// components/ModalMedicationEmergency.jsx
...
const ModalMedicationEmergency = ({ 
  show, 
  onClose, 
  emergencyRoomCareId, 
  onSuccess, 
  ...
  const loadExistingMedications = async () => {
    try {
      ...
      const mappedMedications = existingMedications.map(existing => {
        ...
        return {
          medicationEmergencyId: existing.medicationEmergencyId,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!emergencyRoomCareId) {
      setError('No se ha seleccionado una atención de emergencia válida');
      setTimeout(() => setError(''), 3000);
      return;
    }
    ...
    try {
      console.log('Emergency Room Care ID:', emergencyRoomCareId);
      
      if (mode === 'edit') {
        // Modo edición: eliminar todos los existentes y crear nuevos
        for (const existing of existingMedications) {
          await medicationEmergencyService.delete(existing.medicationEmergencyId);
        }
      }
      ...
      for (const med of selectedMedications) {
        ...
        await medicationEmergencyService.create(payload);
      }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-gray-900">
                {mode === 'edit' ? 'Editar Receta de Emergencia' : 'Recetar Medicamentos (Emergencia)'}
              </h2>
            </div>
            <button
              onClick={handleClose}
...
export default ModalMedicationEmergency;